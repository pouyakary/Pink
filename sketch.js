...
const HIGHLIGHT_COLOR_LIGHT = [199, 21, 133];
const HIGHLIGHT_COLOR_DARK = [90, 37, 161];
const DRAW_COLOR_LIGHT = [156, 17, 89];
...
var colorLayerNeedsFullRedraw = true;
var overlayLayer = null;
var lastKnownDarkMode = null;
var lastKnownEraseMode = null;
...
class Model {
  ...
  addToBuffer(x, y) {
    ...
    drawLatestSegmentOnColorLayer(shape);
  }
...
function createColorLayer() {
  ...
  colorLayerNeedsFullRedraw = true;
}
...
function createOverlayLayer() {
  overlayLayer = ensureGraphicsLayer(
    overlayLayer,
    undefined,
...
function createGlitterLayer() {
  glitterLayer = ensureGraphicsLayer(
    glitterLayer,
    WEBGL,
    configureGlitterLayer
  );
}
...
function scheduleColorLayerRebuild() {
  colorLayerNeedsFullRedraw = true;
}

function getBaseStrokeColor() {
  if (eraseMode) {
    return darkMode ? ERASE_BASE_COLOR_DARK : ERASE_BASE_COLOR_LIGHT;
  }
  return darkMode ? DRAW_COLOR_DARK : DRAW_COLOR_LIGHT;
}

function getHoverHighlightColor() {
  return darkMode ? HIGHLIGHT_COLOR_DARK : HIGHLIGHT_COLOR_LIGHT;
}

function normalizeColorTriplet(color) {
  return [color[0] / 255, color[1] / 255, color[2] / 255];
}

function colorToRgbaString(color, alpha = 1.0) {
...
function applyBaseStrokeColor(layer) {
  const color = getBaseStrokeColor();
...
function drawShapeOnLayer(layer, shape) {
  if (!layer || !shape || shape.size === 0) {
    return;
  }
...
function rebuildColorLayerFromModel() {
  if (!colorLayer || !model) {
    return;
  }
  ...
  colorLayer.clear();
  configureColorLayer();
  applyBaseStrokeColor(colorLayer);

  for (const shape of model.shapes) {
    drawShapeOnLayer(colorLayer, shape);
  }

  colorLayerNeedsFullRedraw = false;
}

function drawLatestSegmentOnColorLayer(shape) {
  if (!colorLayer || !shape || shape.size === 0) {
    return;
  }
  ...
  const [x1, y1] = shape.points[lastIndex - 1];
  const [x2, y2] = shape.points[lastIndex];
  colorLayer.line(x1, y1, x2, y2);
}

function ensureColorLayerIsUpToDate() {
  if (!colorLayer || !model) {
    return;
  }

  if (colorLayerNeedsFullRedraw) {
    rebuildColorLayerFromModel();
  }
}

function renderColorLayer() {
  ensureColorLayerIsUpToDate();
}
...
function renderGlitterLayer() {
  ...
  const hoverColor = normalizeColorTriplet(getHoverHighlightColor());
  const hoverActive = !eraseMode && isMouseInsideCanvas();
  glitterShader.setUniform("uHoverEnabled", hoverActive ? 1.0 : 0.0);
  glitterShader.setUniform("uHoverCenter", [mouseX, mouseY]);
  glitterShader.setUniform("uHoverRadius", MOUSE_HOVER_SENSITIVITY);
  glitterShader.setUniform("uHoverColor", hoverColor);
...
function draw() {
  ...
  } else if (lastKnownEraseMode !== eraseMode) {
    lastKnownEraseMode = eraseMode;
    scheduleColorLayerRebuild();
  }
  ...
  const canRenderGlitter =
    glitterLayer && glitterShader && maskLayer && colorLayer;
  if (canRenderGlitter) {
    renderGlitterLayer();
...
function shouldNotHandleTheMouse() {
  return (
    htmlSectionIsActive || !focused || helpPageIsOpen || locked || dialogIsOpen
  );
}
...
function decideColor(x, y, shouldAllShapeBeSelected, target = null) {
  ...
  const eraseNeutralColor = darkMode
    ? ERASE_BASE_COLOR_DARK
    : ERASE_BASE_COLOR_LIGHT;
  const eraseSelectedColor = darkMode
    ? ERASE_SELECTED_COLOR_DARK
    : ERASE_SELECTED_COLOR_LIGHT;
...
function renderHelpPageBasedOnState() {
  document.getElementById("help-screen").style.display = helpPageIsOpen
    ? "flex"
    : "none";
}
...
function renderHoverHighlights() {
  if (!overlayLayer || !model || eraseMode) {
    return;
  }

  const canRenderHoverInShader = glitterLayer && glitterShader;
  if (canRenderHoverInShader) {
    return;
  }

  overlayLayer.strokeWeight(FACTORY_STROKE_SIZE);
  const highlightColor = getHoverHighlightColor();
  const baseColor = getBaseStrokeColor();
  const drawingContext = overlayLayer.drawingContext;
  const previousStrokeStyle = drawingContext.strokeStyle;
  const gradient = drawingContext.createRadialGradient(
    ...
    mouseX,
    mouseY,
    MOUSE_HOVER_SENSITIVITY
  );
  gradient.addColorStop(0, colorToRgbaString(highlightColor));
  gradient.addColorStop(1, colorToRgbaString(baseColor));
  ...
  for (const shape of model.shapes) {
    ...
    if (points.length === 1) {
      const [x, y] = points[0];
      if (length(x, y, mouseX, mouseY) < MOUSE_HOVER_SENSITIVITY) {
        overlayLayer.point(x, y);
      }
      continue;
    }
    ...
    for (let index = 1; index < points.length; index++) {
      const [x, y] = points[index];
      if (
        length(x, y, mouseX, mouseY) < MOUSE_HOVER_SENSITIVITY ||
        length(px, py, mouseX, mouseY) < MOUSE_HOVER_SENSITIVITY
      ) {
        overlayLayer.line(px, py, x, y);
      }
      px = x;
      py = y;
    }
  ...
  drawingContext.strokeStyle = previousStrokeStyle;
}

function renderEraseModeHighlight() {
  if (
    !overlayLayer ||
    !model ||
    !eraseMode ||
  ...
  const selectedColor = darkMode
    ? ERASE_SELECTED_COLOR_DARK
    : ERASE_SELECTED_COLOR_LIGHT;
  overlayLayer.stroke(selectedColor[0], selectedColor[1], selectedColor[2]);
  overlayLayer.strokeWeight(FACTORY_STROKE_SIZE);
  drawShapeOnLayer(overlayLayer, shape);
}
...
function drawSelectionBox(x, y, width, height) {
  ...
  overlayLayer.line(
    topLeftX,
    topLeftY,
    topLeftX + SELECTION_BOX_CORNER_SIZE,
    topLeftY
  );
  overlayLayer.line(
    topLeftX,
    topLeftY,
    topLeftX,
    topLeftY + SELECTION_BOX_CORNER_SIZE
  );

  const topRightX = topLeftX + 2 * SELECTION_PADDING + width;
  const topRightY = topLeftY;
  overlayLayer.line(
    topRightX - SELECTION_BOX_CORNER_SIZE,
    topRightY,
    topRightX,
    topRightY
  );
  overlayLayer.line(
    ...
    topRightY,
    topRightX,
    topRightY + SELECTION_BOX_CORNER_SIZE
  );
  ...
  overlayLayer.line(
    bottomLeftX,
    bottomLeftY,
    bottomLeftX + SELECTION_BOX_CORNER_SIZE,
    bottomLeftY
  );
  overlayLayer.line(
    bottomLeftX,
    bottomLeftY - SELECTION_BOX_CORNER_SIZE,
    bottomLeftX,
    bottomLeftY
  );
  ...
  overlayLayer.line(
    bottomRightX,
    bottomRightY,
    bottomRightX - SELECTION_BOX_CORNER_SIZE,
    bottomRightY
  );
  overlayLayer.line(
    bottomRightX,
    bottomRightY - SELECTION_BOX_CORNER_SIZE,
    bottomRightX,
    bottomRightY
  );
}
...
function renderOverlay() {
  ...
  renderEraseModeHighlight();

  if (
    somethingIsSelected &&
    (eraseMode || !shouldActOnMouseHover) &&
    selectedShapeIndex >= 0
  ) {
    const shape = model.shapes[selectedShapeIndex];
    if (shape) {
